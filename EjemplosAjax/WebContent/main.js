...
function rellenaCaja()
{
	...
	xmlHttp.open('GET', 'CargaMenuSelect', true); //indicamos como vamos a enviar los datos, en este caso con el metodo GET al archivo el valor que le indiquemos en el textbox
	xmlHttp.send(null);
      ...
      if(xmlHttp.status==200)// y el código de la cabecera http es bueno
          {
        	...
        	var lista_empleados = xmlDoc.getElementsByTagName("empleado");
          	...
          	// esta parte debe borrar la tabla que pudiese haber para luego construir la nueva 
//          	for (var j=0; j <  ?? ; j++)
//          	{
//          		tabla.remove(0);// esto no vale claro
//          	}
          	//document.write("<table><tr><th>Id</th><th>Nombre</th></tr>");
          	
			for (var i= 0; i < nelementos; i++) {
				
				id = lista_empleados[i].getElementsByTagName("id")[0].data;
				nombre = lista_empleados[i].getElementsByTagName("nombre")[0].data;
